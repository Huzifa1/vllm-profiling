...
class Worker(WorkerBase):
    ...
    def initialize_from_config(self, kv_cache_config: KVCacheConfig) -> None:
        ...
        if self.vllm_config.model_config.enable_sleep_mode:
            ...
            context = nullcontext()
        with context:
            init_kv_cache_start = time.perf_counter()
            self.model_runner.initialize_kv_cache(kv_cache_config)
            init_kv_cache_end = time.perf_counter()
            logger.info(("NEW: Initializing KV Cache took %.2f seconds"), init_kv_cache_end - init_kv_cache_start)
